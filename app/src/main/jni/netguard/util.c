...
int is_http_port(int port) {
    int http_ports[] = {80, 8080, 8000};
    int num_http_ports = sizeof(http_ports)/sizeof(http_ports[0]);
    int i;
    for(i=0; i<num_http_ports; i++) {
        if(port == http_ports[i]) return 1;
    }
    return 0;
...
int is_https_port(int port) {
    int https_ports[] = {443};
    int num_http_ports = sizeof(https_ports)/sizeof(https_ports[0]);
    int i;
    for(i=0; i<num_http_ports; i++) {
        if(port == https_ports[i]) return 1;
    }
    return 0;
...
void update_stdv(struct stdv_state *ss, int data) {
    ss->n++;
    float delta = data - ss->mean;
    ss->mean += delta/ss->n;
    float delta2 = data - ss->mean;
    ss->M2 += delta*delta2;
}