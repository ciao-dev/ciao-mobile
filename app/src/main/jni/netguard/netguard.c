...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    const char *packet = "com/isthatfreeproxysafe/ciao/Packet";
    clsPacket = jniGlobalRef(env, jniFindClass(env, packet));

    const char *allowed = "com/isthatfreeproxysafe/ciao/Allowed";
    clsAllowed = jniGlobalRef(env, jniFindClass(env, allowed));

    const char *rr = "com/isthatfreeproxysafe/ciao/ResourceRecord";
    clsRR = jniGlobalRef(env, jniFindClass(env, rr));

    const char *usage = "com/isthatfreeproxysafe/ciao/Usage";
    clsUsage = jniGlobalRef(env, jniFindClass(env, usage));

    const char *proxy_usage = "com/isthatfreeproxysafe/ciao/ProxyUsage";
    clsProxyUsage = jniGlobalRef(env, jniFindClass(env, proxy_usage));
...
Java_com_isthatfreeproxysafe_ciao_ServiceSinkhole_jni_1init(JNIEnv *env, jobject instance) {
...
JNIEXPORT void JNICALL
Java_com_isthatfreeproxysafe_ciao_ServiceSinkhole_jni_1start(
...
JNIEXPORT void JNICALL
Java_com_isthatfreeproxysafe_ciao_ServiceSinkhole_jni_1stop(
...
Java_com_isthatfreeproxysafe_ciao_ServiceSinkhole_jni_1get_1mtu(JNIEnv *env, jobject instance) {
    return get_mtu();
}


JNIEXPORT jintArray JNICALL
Java_com_isthatfreeproxysafe_ciao_ServiceSinkhole_jni_1get_1proxy_1stats(JNIEnv *env, jobject instance) {
        ...
        if (s->protocol == IPPROTO_TCP) {
            if (s->tcp.state != TCP_CLOSING && s->tcp.state != TCP_CLOSE) {
                ...
                if(s->tcp.proxy != NULL) {
                    ...
                    if (is_http_port(dest_port)) {
                        jcount[1]++;
                        jcount[2] += (now - s->tcp.time);
                    }
                    else if (is_https_port(dest_port)) {
                        jcount[3]++;
                        jcount[4] += (now - s->tcp.time);
                    }
                }
            }
        }
    ...
    if(jcount[1] != 0) jcount[2] /= jcount[1];
    if(jcount[3] != 0) jcount[4] /= jcount[3];
...
Java_com_isthatfreeproxysafe_ciao_ServiceSinkhole_jni_1is_1http_1port(JNIEnv *env, jobject instance, jint port) {

    return (jboolean) (is_http_port((int) port));
}


JNIEXPORT jboolean JNICALL
Java_com_isthatfreeproxysafe_ciao_ServiceSinkhole_jni_1is_1https_1port(JNIEnv *env, jobject instance, jint port) {

    return (jboolean) (is_https_port((int) port));
}


JNIEXPORT jintArray JNICALL
Java_com_isthatfreeproxysafe_ciao_ServiceSinkhole_jni_1get_1stats(JNIEnv *env, jobject instance) {
    if (pthread_mutex_lock(&lock))
        log_android(ANDROID_LOG_ERROR, "pthread_mutex_lock failed");
...
JNIEXPORT void JNICALL
Java_com_isthatfreeproxysafe_ciao_ServiceSinkhole_jni_1pcap(
...
JNIEXPORT void JNICALL
Java_com_isthatfreeproxysafe_ciao_ServiceSinkhole_jni_1socks5(JNIEnv *env, jobject instance, jstring addr_,
...
JNIEXPORT void JNICALL
Java_com_isthatfreeproxysafe_ciao_ServiceSinkhole_jni_1done(JNIEnv *env, jobject instance) {
...
Java_com_isthatfreeproxysafe_ciao_Util_jni_1getprop(JNIEnv *env, jclass type, jstring name_) {
    const char *name = (*env)->GetStringUTFChars(env, name_, 0);
...
JNIEXPORT jboolean JNICALL
Java_com_isthatfreeproxysafe_ciao_Util_is_1numeric_1address(JNIEnv *env, jclass type, jstring ip_) {
    jboolean numeric = 0;
...
void log_packet(const struct arguments *args, jobject jpacket) {
    ...
    const char *signature = "(Lcom/isthatfreeproxysafe/ciao/Packet;)V";
...
void dns_resolved(const struct arguments *args,
    ...
    const char *signature = "(Lcom/isthatfreeproxysafe/ciao/ResourceRecord;)V";
...
jfieldID fidRedirect = NULL;
jfieldID fidMasked = NULL;
...
struct allowed *is_address_allowed(const struct arguments *args, jobject jpacket) {
    ...
    const char *signature = "(Lcom/isthatfreeproxysafe/ciao/Packet;)Lcom/isthatfreeproxysafe/ciao/Allowed;";
    ...
    if (jallowed != NULL) {
        if (fidRaddr == NULL) {
            ...
            fidMasked = jniGetFieldID(args->env, clsAllowed, "masked", "Z");
        }
        ...
        allowed.rport = (uint16_t) (*args->env)->GetIntField(args->env, jallowed, fidRport);
        allowed.rport = (uint16_t) (*args->env)->GetIntField(args->env, jallowed, fidRport);
        allowed.redirect = (uint8_t) (*args->env)->GetBooleanField(args->env, jallowed, fidRedirect);
        allowed.masked = (uint8_t) (*args->env)->GetBooleanField(args->env, jallowed, fidMasked);
...
void account_usage(const struct arguments *args, jint version, jint protocol,
    ...
    const char *signature = "(Lcom/isthatfreeproxysafe/ciao/Usage;)V";
