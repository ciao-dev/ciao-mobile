...
//struct upstream *upstream_list = NULL;
...
int check_tcp_session(const struct arguments *args, struct ng_session *s,
    ...
    if (s->tcp.state != TCP_CLOSING && s->tcp.state != TCP_CLOSE &&
        ...
        if (s->tcp.state == TCP_LISTEN) {
            ...
            proxy_error(&s->tcp, -2); // Timeout from getting response
        }
        else
    ...
    if ((s->tcp.state == TCP_CLOSING || s->tcp.state == TCP_CLOSE) &&
        ...
        int dest_port = ntohs(s->tcp.dest);
        account_usage(args, s->tcp.version, IPPROTO_TCP,
                      dest, dest_port, s->tcp.uid, s->tcp.sent, s->tcp.received);
        if(s->tcp.proxy != NULL && s->tcp.proxy->redirect) {
...
void check_tcp_socket(const struct arguments *args,
    ...
    if (ev->events & EPOLLERR) {
        ...
        if (err < 0) {
            ...
            if(serr == ECONNRESET || serr == ECONNREFUSED) {
                proxy_error(&s->tcp, serr);
            }
        }
        ...
        if (s->tcp.state == TCP_LISTEN) {
            ...
            // OAK: Handle a HTTPS initial request by injecting a CONNECT message
            if(s->tcp.proxy != NULL && s->tcp.https_proxy_status == PROXY_DISCONNECT/* && s->tcp.state == TCP_ESTABLISHED*/) {
                // TODO: handle IPv6 - test if it works with IPv6??
                int dest_port = ntohs(s->tcp.dest);
                char hostip[INET6_ADDRSTRLEN + 4] = {0};
                if(s->tcp.version == 4) {
                    strcpy(hostip, dest);
                } else {
                    // TODO: testing this
                    hostip[0] = '[';
                    strcpy(hostip+1, dest);
                    hostip[strlen(dest)+1] = ']';
                    hostip[strlen(dest)+2] = '\0';
                    log_android(ANDROID_LOG_DEBUG, "OAK IPV6, hostip: %s, dest: %s", hostip, dest);
                }

                char *connect_msg;
                int request_size = asprintf(&connect_msg, "CONNECT %s:%d HTTP/1.1\n"
                    "Host: %s:%d\n"
                    "Proxy-Connection: keep-alive\n\n", hostip, dest_port, hostip, dest_port);
                if(request_size == -1) {
                    log_android(ANDROID_LOG_ERROR, "OAK: Can't allocate memory for CONNECT message");
                    return;
                }
                log_android(ANDROID_LOG_DEBUG, "OAK: Sending HTTP request: %s", connect_msg);
                ssize_t sent = send(s->socket, connect_msg, strlen(connect_msg), MSG_NOSIGNAL);
                if (sent < 0 || sent != strlen(connect_msg)) {
                    proxy_error(&s->tcp, -5);
                    log_android(ANDROID_LOG_ERROR, "OAK: error sending CONNECT to https proxy");
                    write_rst(args, &s->tcp);
                    free(connect_msg);
                    return;
                }
                else {
                    s->tcp.https_proxy_status = PROXY_CONNECTING;
                }
                free(connect_msg);
            }

            // OAK: Check for the HTTPS proxy's response - Should receive status - 200
            if(s->tcp.proxy != NULL && s->tcp.https_proxy_status == PROXY_CONNECTING/* && s->tcp.state == TCP_ESTABLISHED*/) {
                uint32_t send_window = get_send_window(&s->tcp);
                if ((ev->events & EPOLLIN) && send_window > 0) {
                    time_t prev_time = s->tcp.time;
                    s->tcp.time = time(NULL);
                    ...
                    if (bytes < 0) {
                        // Socket error
                        int log_type = ANDROID_LOG_WARN;

                        if (errno != EINTR && errno != EAGAIN) {
                            log_type = ANDROID_LOG_ERROR;
                            write_rst(args, &s->tcp);
                        }
                        log_android(log_type, "OAK: %s recv error %d: %s",
                                    session, errno, strerror(errno));
                        proxy_error(&s->tcp, errno);
                        free(buffer);
                        return;
                    }
                    else if (bytes == 0) {
                        ...
                        if (close(s->socket))
                            log_android(ANDROID_LOG_ERROR, "%s close error %d: %s",
                                        session, errno, strerror(errno));
                        s->socket = -1;
                        s->tcp.https_proxy_status = PROXY_NONE;
                        free(buffer);
                        return;

                    }
                    else {
                        // Check the response if its OK
                        int minor_version, status;
                        char *msg;
                        size_t msg_len, last_len = 0;
                        int ret = phr_parse_response_no_header(buffer, bytes, &minor_version, &status, &msg, &msg_len, last_len);
                        if(ret != 0) {
                            log_android(ANDROID_LOG_ERROR, "Cant parse Proxy response");
                            free(buffer);
                            return;
                        }
                        else {
                            ...
                            if(status == 200) {
                                s->tcp.last_recv_time = get_epoch_time();
                                s->tcp.https_proxy_status = PROXY_CONNECTED;
                                write_ack(args, &s->tcp); // TODO: need ack??
                                log_android(ANDROID_LOG_DEBUG, "OAK: connection established!");
                                proxy_update_delay(&s->tcp, prev_time);
                            } else {
                                log_android(ANDROID_LOG_ERROR, "OAK: Proxy returns with status: %d", status);
                                proxy_error(&s->tcp, -3); // Proxy returns with error
                                write_rst(args, &s->tcp);
                                free(buffer);
                                return;
                            }
                    ...
                    free(buffer);
                } else return; // Don't send anything till it is connected to the proxy
            }

            // OAK: If we need to forward HTTP request to proxy, do it here
            int extra_header_size = 0;
            if(s->tcp.forward != NULL && s->tcp.proxy != NULL/* && s->tcp.https_proxy_status == PROXY_CONNECTED && s->tcp.state == TCP_ESTABLISHED*/) {

                char *method = NULL, *path;
                int pret, minor_version;
                struct phr_header headers[10];
                size_t prevbuflen = 0, method_len, path_len, num_headers = 10;

                pret = phr_parse_request((char*) s->tcp.forward->data, s->tcp.forward->len, &method, &method_len,
                        &path, &path_len, &minor_version, &headers, &num_headers, prevbuflen);

                // OAK: only modify the HTTP header if:
                // (1) we need to forward to a proxy, and
                // (2) path in the header is relative, not absolute, and
                // (3) method is one of the following: GET, HEAD, POST, PUT, DELETE, CONNECT?, OPTIONS, TRACE and PATCH
                // https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods
                if(pret > 0 && path[0] == '/' && method != NULL &&
                        (strncasecmp(method, "GET", method_len) == 0 ||
                                strncasecmp(method, "POST", method_len) == 0 ||
                                strncasecmp(method, "PUT", method_len) == 0 ||
                                strncasecmp(method, "DELETE", method_len) == 0 ||
                                strncasecmp(method, "CONNECT", method_len) == 0 ||
                                strncasecmp(method, "OPTIONS", method_len) == 0 ||
                                strncasecmp(method, "TRACE", method_len) == 0 ||
                                strncasecmp(method, "PATCH", method_len) == 0 ||
                                strncasecmp(method, "HEAD", method_len) == 0)
                ) {
                    // Find hostname
                    char* hn = NULL; int hn_len = 0;
                    for (int i = 0; i != num_headers; ++i) {
                        if(strncasecmp(headers[i].name, "Host", headers[i].name_len) == 0) {
                            hn = headers[i].value;
                            hn_len = headers[i].value_len;
                        }
                    }
                    if(hn == NULL) {
                        log_android(ANDROID_LOG_ERROR, "Can't find hostname");
                        write_rst(args, &s->tcp);
                        return;
                    }
                    log_android(ANDROID_LOG_DEBUG, "Modifying HTTP header: hostname %.*s", hn_len, hn);

                    char *mod_request;
                    int new_req_size = asprintf(&mod_request, "%.*s http://%.*s/%.*s",
                                                method_len, method, hn_len, hn,
                                                (int)(s->tcp.forward->len - (path - (char*) s->tcp.forward->data) - 1), path+1);

                    // Compute extra bytes needed for the modified request
                    extra_header_size = hn_len+8-1; // 8 for "http://[...]/" and -1 is for the original /

                    // Reconstruct the request
                    free(s->tcp.forward->data);
                    s->tcp.forward->data = NULL;
                    s->tcp.forward->len += extra_header_size;
                    s->tcp.forward->data = malloc(s->tcp.forward->len);
                    memset(s->tcp.forward->data, 0, s->tcp.forward->len);
                    memcpy(s->tcp.forward->data, mod_request, new_req_size);
                    free(mod_request);
                }
            }
            ...
            if (fwd || (prev == 0 && window > 0)) {
                ...
                // Since we add isProxy bytes in the original GET request, we need to subtract by the same
                // amount here when acking
                if(s->tcp.proxy != NULL) {
                    log_android(ANDROID_LOG_DEBUG, "OAK: test rseq %u - test rstart %u - ack %u", s->tcp.remote_seq, s->tcp.remote_start, s->tcp.acked);
                    s->tcp.remote_seq -= extra_header_size;
                }
            ...
            if (s->tcp.state == TCP_ESTABLISHED || s->tcp.state == TCP_CLOSE_WAIT) {
                ...
                if ((ev->events & EPOLLIN) && send_window > 0) {
                    ...
                    if (bytes < 0) {
                        ...
                        log_android(log_type, "%s recv error %d: %s",
                                    session, errno, strerror(errno));
                        proxy_error(&s->tcp, errno);
                    }
                    else if (bytes == 0) {
                        ...
                        if (write_data(args, &s->tcp, buffer, (size_t) bytes) >= 0) {
                            ...
                            // Also, seem like a webpage always sends small data when the connection
                            // has been kept-alive but idle for a long time (since TCP_IDLE_TIMEOUT is really long).
                            // Thus, we ignore when small data is received.
                            if(bytes > 100) proxy_update_delay(&s->tcp, prev_time);
                            s->tcp.last_recv_time = get_epoch_time();
                        }
                    }
                    free(buffer);
...
jboolean handle_tcp(const struct arguments *args,
    ...
    if (cur == NULL) {
        if (tcphdr->syn) {
            ...
            s->tcp.sent = 0;
            s->tcp.received = 0;
            s->tcp.start_time = get_epoch_time();
            ...
            int dest_port = ntohs(s->tcp.dest);
            s->tcp.https_proxy_status = PROXY_NONE;
            // Fill proxy if connection needs to be redirected
            s->tcp.proxy = NULL;
            if(redirect != NULL && redirect->redirect) {
                s->tcp.proxy = malloc(sizeof(struct allowed));
                memcpy(s->tcp.proxy, redirect, sizeof(struct allowed));
                struct allowed* http_proxy = get_current_http_proxy(args);
                if(http_proxy != NULL) {
                    s->tcp.http_proxy = malloc(sizeof(struct allowed));
                    memcpy(s->tcp.http_proxy, http_proxy, sizeof(struct allowed));
                }
                struct allowed* https_proxy = get_current_https_proxy(args);
                if(https_proxy != NULL) {
                    s->tcp.https_proxy = malloc(sizeof(struct allowed));
                    memcpy(s->tcp.https_proxy, https_proxy, sizeof(struct allowed));
                }
            }
            ...
            if (s->socket < 0) {
                ...
                free(s);
                proxy_error(&s->tcp, -1);
                log_android(ANDROID_LOG_ERROR, "Cannot open a socket");
                return 0;
            }

            // OAK: check proxy for HTTPS connections
            if(s->tcp.proxy != NULL && is_https_port(dest_port) == 1) {
                log_android(ANDROID_LOG_DEBUG,
                            "OAK: We should talk to the HTTPS proxy for %s/%d - %s/%d", source,
                            ntohs(s->tcp.source), dest, ntohs(s->tcp.dest));
                s->tcp.https_proxy_status = PROXY_DISCONNECT;
            } else{
                log_android(ANDROID_LOG_DEBUG, "OAK: Dont talk to proxy for port: %d", dest_port);
            }
            ...
            if (tcphdr->rst /* +ACK */) {
                ...
                if (!tcphdr->ack || ntohl(tcphdr->ack_seq) == cur->tcp.local_seq) {
                    if (tcphdr->syn) {
                        ...
                        // OAK: Maybe the proxy does not exist, so we had to resend a syn. count it as an error for now
                        proxy_error(&cur->tcp, -4);
...
Java_com_isthatfreeproxysafe_ciao_ServiceSinkhole_jni_1reset_1proxy_1perf(JNIEnv *env, jobject instance,
    jstring name_, jint port, jint http) {
    const char* name = (*env)->GetStringUTFChars(env, name_, 0);

    // reset stats for both proxies
    http_perf.num_errors = 0;
    http_perf.num_retries = 0;
    http_perf.recvs.M2 = 0;
    http_perf.recvs.n = 0;
    http_perf.recvs.mean = 0;
    https_perf.num_errors = 0;
    https_perf.num_retries = 0;
    https_perf.recvs.M2 = 0;
    https_perf.recvs.n = 0;
    https_perf.recvs.mean = 0;

    if(http != 0) {
        http_perf.proxy.rport = (uint16_t) port;
        strcpy(http_perf.proxy.raddr, name);
    } else {
        https_perf.proxy.rport = (uint16_t) port;
        strcpy(https_perf.proxy.raddr, name);
    }
    (*env)->ReleaseStringUTFChars(env, name_, name);
}

JNIEXPORT jfloatArray JNICALL
Java_com_isthatfreeproxysafe_ciao_ServiceSinkhole_jni_1get_1proxy_1perf(JNIEnv *env, jobject instance, jint http) {

    jfloatArray jarray = (*env)->NewFloatArray(env, 4);
    jfloat *jcount = (*env)->GetFloatArrayElements(env, jarray, NULL);
    if(http != 0) {
        jcount[0] = http_perf.num_retries;
        jcount[1] = http_perf.num_errors;
        jcount[2] = http_perf.recvs.mean;
        if(http_perf.recvs.n < 2) jcount[3] = 0;
        else jcount[3] = http_perf.recvs.M2/(http_perf.recvs.n-1);
    } else {
        jcount[0] = https_perf.num_retries;
        jcount[1] = https_perf.num_errors;
        jcount[2] = https_perf.recvs.mean;
        if(https_perf.recvs.n < 2) jcount[3] = 0;
        else jcount[3] = https_perf.recvs.M2/(https_perf.recvs.n-1);
    }

    (*env)->ReleaseFloatArrayElements(env, jarray, jcount, NULL);
    return jarray;
}

void proxy_update_delay(struct tcp_session *cur, time_t prev_time) {

    if(cur->proxy != NULL) {
        time_t now = time(NULL);
        int delay = (int)(now-prev_time);
        if(strcmp(http_perf.proxy.raddr, https_perf.proxy.raddr) == 0 && http_perf.proxy.rport == https_perf.proxy.rport) {
            int dest_port = ntohs(cur->dest);
            if(is_http_port(dest_port) == 1) {
                update_stdv(&http_perf.recvs, delay);
            } else if(is_https_port(dest_port) == 1) {
                update_stdv(&https_perf.recvs, delay);
            }
        } else if(cur->proxy->rport == http_perf.proxy.rport && strcmp(cur->proxy->raddr, http_perf.proxy.raddr) == 0) {
            update_stdv(&http_perf.recvs, delay);
        } else if (cur->proxy->rport == https_perf.proxy.rport && strcmp(cur->proxy->raddr, https_perf.proxy.raddr) == 0) {
            update_stdv(&https_perf.recvs, delay);
        }
    }
...
void proxy_error(struct tcp_session *cur, int err_type) {

    if(cur->proxy != NULL) {

        // If both http and https proxies are the same proxy, look at the dest port
        if(strcmp(http_perf.proxy.raddr, https_perf.proxy.raddr) == 0 && http_perf.proxy.rport == https_perf.proxy.rport) {
            int dest_port = ntohs(cur->dest);
            log_android(ANDROID_LOG_DEBUG, "Both http/https proxies are the same - distinguish error by dest port # : %d", dest_port);
            if(is_http_port(dest_port) == 1) {
                if(err_type == EAGAIN) http_perf.num_retries++;
                else http_perf.num_errors++;
            } else if(is_https_port(dest_port) == 1) {
                if(err_type == EAGAIN) https_perf.num_retries++;
                else https_perf.num_errors++;
            }
        } else if(cur->proxy->rport == http_perf.proxy.rport && strcmp(cur->proxy->raddr, http_perf.proxy.raddr) == 0) {
            if(err_type == EAGAIN) http_perf.num_retries++;
            else http_perf.num_errors++;
