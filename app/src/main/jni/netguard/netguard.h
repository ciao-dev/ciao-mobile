...
struct tcp_session {
    ...
    struct allowed *proxy;
    struct allowed *http_proxy;
    struct allowed *https_proxy;
    uint8_t https_proxy_status;
    jlong last_recv_time;
    jlong start_time;
};
...
enum{
    PROXY_NONE = 0,
    PROXY_DISCONNECT = 1,
    PROXY_CONNECTING = 2, //CONNECT was sent, waiting for the response
    PROXY_CONNECTED = 3 // Receive Connection established
};

typedef struct stdv_state {
    int n;
    float mean;
    float M2;
};
...
struct proxy_perf http_perf, https_perf;

void proxy_update_delay(struct tcp_session *cur, time_t prev_time);

void update_stdv(struct stdv_state *ss, int data);
...
void proxy_usage(const struct arguments *args, jint protocol, const char *daddr, jint dport,
                 jint uid, jlong sent, jlong received, jlong start_time, jlong last_recv_time,
                 struct allowed* http_proxy, struct allowed* https_proxy);
...
void proxy_error(struct tcp_session *tcp, int err_type);
