...
jboolean handle_udp(const struct arguments *args,
    ...
    if (cur == NULL) {
        ...
        if (redirect == NULL || !redirect->redirect)
            rversion2 = s->udp.version;
        else
            rversion2 = (strstr(redirect->raddr, ":") == NULL ? 4 : 6);
        s->udp.mss = (uint16_t) (rversion2 == 4 ? UDP4_MAXMSG : UDP6_MAXMSG);
        ...
        log_android(ANDROID_LOG_DEBUG, "OAK: open UDP socket %d", s->socket);
    ...
    log_android(ANDROID_LOG_INFO, "UDP forward from tun %s/%u to %s/%u data %d at socket %d",
                source, ntohs(udphdr->source), dest, ntohs(udphdr->dest), datalen, cur->socket);
    ...
    /*struct sockaddr_in serv_addr, test_addr;
    int testaddrlen = sizeof(test_addr);
    //if(getsockname(cur->socket, (struct sockaddr *)&test_addr, &testaddrlen) != 0) {
        log_android(ANDROID_LOG_DEBUG, "OAK: UDP original bound ip %s, port %d",
                    inet_ntoa(test_addr.sin_addr), ntohs(test_addr.sin_port));

        serv_addr.sin_family = AF_INET;
        struct in_addr ip_addr;
        ip_addr.s_addr = cur->udp.saddr.ip4;

        char* ss = inet_ntoa(ip_addr);
        log_android(ANDROID_LOG_DEBUG, "OAK: UDP src ip: %s", ss);
        serv_addr.sin_addr.s_addr = inet_addr("10.1.10.1");
        serv_addr.sin_port = 0; //htons(2222);
        //inet_pton(AF_INET, ss, &serv_addr.sin_addr);
        if (bind(cur->socket, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0) {
            log_android(ANDROID_LOG_ERROR, "OAK: UDP cant bind");
        }
    //} else {
        log_android(ANDROID_LOG_DEBUG, "OAK: UDP already bound at ip %s, port %d",
